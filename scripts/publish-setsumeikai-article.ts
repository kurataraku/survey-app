/**
 * 説明会スケジュール記事を articles に公開挿入する。
 *   npx tsx scripts/publish-setsumeikai-article.ts --slug=kumamoto-tsushin-setsumeikai-2026-schedule
 */
...
const SCHOOL_SLUGS_BY_ARTICLE: Record<string, string[]> = {
  'kumamoto-tsushin-setsumeikai-2026-schedule': [
    'yushi-kokusai-kuchikomi',
    'oozora-kuchikomi',
    'try-gakuin-kuchikomi',
    'daiichi-gakuin-kuchikomi',
    'clark-kuchikomi',
    'asukamirai-kizuna-kuchikomi',
  ],
  'miyagi-tsushin-setsumeikai-2026-schedule': [
    ...
    'try-gakuin-kuchikomi',
    'daiichi-gakuin-kuchikomi',
    'seisa-kokusai-kuchikomi',
    'human-campus-koukou-kuchikomi',
  ],
};
...
async function main() {
  const { slug: SLUG } = parseArgs();
  const SCHOOL_SLUGS = SCHOOL_SLUGS_BY_ARTICLE[SLUG];
  if (!SCHOOL_SLUGS) {
    throw new Error(`未登録の slug です: ${SLUG}`);
  }
  ...
  const draftPath = path.join(process.cwd(), '記事下書き', `${SLUG}.md`);
