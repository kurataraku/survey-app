...
import { runRewriter } from '@/lib/seo-generation/rewriter';
...
export async function POST(
  ...
  try {
    ...
    const { data: draft, error: fetchError } = await supabase
      .from('seo_drafts')
      .select('id, keyword, title, body_md, quality_score, seo_meta, tokens_used')
      .eq('id', id)
      .single();
    ...
    if (!draft.quality_score) {
      return NextResponse.json(
        { error: '品質スコアがありません。先にverifyを実行してください' },
        { status: 400 }
      );
    }
    ...
    await supabase
      .from('seo_drafts')
      .update({ current_step: 'rewrite', status: 'generating' })
      .eq('id', id);
    ...
    const message = error instanceof Error ? error.message : 'Rewriter実行エラー';
    ...
    await supabase
      .from('seo_drafts')
      .update({ status: 'failed', error_message: message, current_step: 'rewrite' })
