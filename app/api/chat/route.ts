      ...
      focusList.length > 1
        ? `今回の相談で気にされているポイントは複数あります。相談文で先に述べられたものほど優先度が高いものとして扱い、すべてのポイントに回答内で必ず触れてください。` +
          focusList
            ...
            .join('') +
          ' 各候補校の説明では、最優先のポイントに直接関係する口コミ根拠を最低1つは明記し、2番目以降のポイントについても口コミ根拠か確認ポイントのどちらかで必ず触れてください。最優先のポイントに関する根拠が薄い学校は、候補にしないか「根拠は弱め」と明記してください。2番目以降のポイントについて参照情報に口コミ根拠が見つからない場合は、曖昧にぼかさず「◯◯に関する口コミ根拠は今回見つからなかったため、見学・説明会での確認ポイントとして挙げます」と正直に明示してください。回答本文では「主訴」「ポイント1」「最優先のポイント」のような内部ラベルや専門用語を使わず、「不登校からの再スタート」「友達づくり」のように内容そのものの言葉で表現してください。'
        : focus
