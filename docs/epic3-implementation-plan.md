...
#### 1.2 記事と学校の関連テーブル（article_schools）の作成
**ファイル**: `supabase-schema-epic3-article-schools.sql`
...
CREATE INDEX idx_article_schools_article_id ON article_schools(article_id);
CREATE INDEX idx_article_schools_school_id ON article_schools(school_id);
CREATE INDEX idx_article_schools_display_order ON article_schools(article_id, display_order);
```

**説明**:
- `display_order`: 記事内での学校の表示順序（0から開始、数値が小さいほど上位）
- `note`: 記事内で学校に対して追加する説明文（例：「学費が特に安い」「口コミ評価が高い」など）
- 記事と学校は多対多の関係（1つの記事に複数の学校を関連付け可能）
...
```json
{
  ...
  "schools": [] // 関連学校の配列（オプション）
}
```

#### 3.3 記事に紐づく学校一覧取得API
**ファイル**: `app/api/articles/[slug]/schools/route.ts`（新規作成）

**エンドポイント**: `GET /api/articles/[slug]/schools`

**機能**:
- 記事に紐づく学校一覧を取得
- 表示順（display_order）でソート
- 学校の集計データ（評価、口コミ数）も含める
...
**エンドポイント**:
- `GET /api/admin/articles/[id]` - 記事取得（非公開も含む、関連学校も含む）
- `PUT /api/admin/articles/[id]` - 記事更新
- `DELETE /api/admin/articles/[id]` - 記事削除

#### 3.5 記事と学校の関連付けAPI
**ファイル**: `app/api/admin/articles/[id]/schools/route.ts`（新規作成）

**エンドポイント**:
- `POST /api/admin/articles/[id]/schools` - 学校の追加
- `DELETE /api/admin/articles/[id]/schools/[school_id]` - 学校の削除
- `PUT /api/admin/articles/[id]/schools/[school_id]` - 表示順・コメントの更新
...
**ファイル**: `components/ArticleSchoolList.tsx`（新規作成）
...
- Markdownコンテンツ本文
- 関連学校の表示（関連学校がある場合、SchoolCardコンポーネント再利用）
- 関連リンク（他の特集ページへのリンク）
...
- `supabase-schema-epic3-article-schools.sql`
...
- `app/api/articles/[slug]/schools/route.ts`
- `app/api/admin/articles/route.ts`
- `app/api/admin/articles/[id]/route.ts`
- `app/api/admin/articles/[id]/schools/route.ts`
...
- `components/ArticleSchoolList.tsx`
...
- `article_schools`テーブル（新規）: Epic3で作成

### データ整合性
- 記事の削除時、記事と学校の関連付け（article_schools）も削除される（CASCADE）
- 学校の削除時、記事との関連付け（article_schools）も削除される（CASCADE）
- スラッグの重複チェックを実装
