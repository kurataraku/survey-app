...
| **ページビュー** | `GoogleAnalytics` クライアントで、**初期表示（フルロード）とクライアント側のページ遷移時**の両方で `page_view` を手動送信（`gtag` は `send_page_view: false` のため自動送信なし）。同一 path の 3 秒以内の重複送信もガード。 |
...
- **二重計測対策**: 当実装では `gtag('config', ..., { send_page_view: false })` としており、**手動で** 初期表示・クライアント遷移の両方で `page_view` を 1 回ずつ送信します。GTM 等の他タグと併用する場合は、二重計測に注意し必要に応じて送信条件を調整してください。
