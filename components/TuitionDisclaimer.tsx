...
export default function TuitionDisclaimer({ className = '', variant = 'full' }: TuitionDisclaimerProps) {
  if (variant === 'lead') {
    return (
      <p
        ...
        role="note"
      >
        掲載している金額は、各学校の公開情報にもとづく<strong className="font-semibold">参考目安</strong>
        です。就学支援金の適用、コース・通学頻度、世帯状況などにより実際の負担額は異なります。
  ...
  return (
    <p className={`text-xs text-gray-500 leading-relaxed ${className}`}>
      ※通信制高校の学費は、学校種別、通学頻度、選択コース、サポート内容、就学支援金の適用状況によって大きく変わります。このページでは、確認できる公開情報をもとに、初年度費用・年間費用などの目安を掲載しています。実際の負担額は家庭の状況や選択コースによって異なるため、最新情報は必ず各学校の公式資料・説明会・個別相談でご確認ください。
    </p>
  );
}
