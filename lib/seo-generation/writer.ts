...
### 表（比較・一覧・スケジュール例など）
- 読者の意思決定に効く**比較・パターン整理**（例: 一日の流れの型、通学頻度の違い、チェック項目の一覧）では、**GitHub Flavored Markdown の表**を**1記事あたり1〜2個**入れてよい
- **表は必ず次の3要素で構成すること（省略厳禁）**: (1)ヘッダ行（列は半角の縦棒で区切る）(2)**すぐ下の行に区切り行**（各列にハイフン3文字以上。例: 3列なら半角で | --- | --- | --- | ）(3)データ行。**区切り行を書かない出力は失格扱いになるので必ず入れること**
- 記載例（この形を必ず踏襲すること。区切り行の直後からデータ行）:
  | モデル | 一日の流れの例 | 向いている人 |
  | --- | --- | --- |
  | 週5通学型 | 8:30〜15:00授業 | 生活リズムを整えたい人 |
- **セル内に改行を入れない**こと
- 表の内容は根拠・口コミの整理に留め、**根拠にない数値調査の捏造や独自ランキング**は書かないこと
...
- 比較に効く箇所では **Markdown の表（縦棒区切り + 区切り行）** を活用してよい（セル内改行は禁止）
